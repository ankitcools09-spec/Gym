...
function GymTracker() {
  ...
  const exportToPDF = async () => {
    try {
      ...
      doc.setFontSize(20);
      doc.text("ANKIT'S GYM LOG", 20, 20);
      
      doc.setFontSize(12);
      let y = 40;
      
      sessions.slice(0, 10).forEach(session => {
        doc.text(`Date: ${new Date(session.date).toLocaleDateString()}`, 20, y);
        y += 10;
        
        if (session.exercises) {
          session.exercises.forEach(ex => {
            doc.setFontSize(10);
            doc.text(`  ${ex.exerciseName}`, 25, y);
            y += 7;
            
            if (ex.sets && ex.sets.length > 0) {
              const setsText = ex.sets.map((set, i) => {
                if (ex.trackingType === 'weight_reps') return `${set.weight}kg×${set.reps}`;
                if (ex.trackingType === 'reps_only') return `${set.reps} reps`;
                if (ex.trackingType === 'duration') return formatDuration(set.duration);
                if (ex.trackingType === 'distance') return formatDistance(set.distance);
                return '';
              }).filter(Boolean).join(', ');
              
              doc.text(`    ${setsText}`, 30, y);
              y += 7;
            }
          });
        }
  ...
  const WorkoutCalendar = () => {
    ...
    const workoutDates = sessions.map(s => new Date(s.date).getDate());
      ...
      <div style={{ background: '#1a1a1a', padding: '20px', borderRadius: '12px', marginBottom: '20px', border: '1px solid #2a2a2a' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '5px', textAlign: 'center' }}>
          ...
          {days.map((day, idx) => (
            <div key={idx} style={{
              padding: '10px',
              background: day && workoutDates.includes(day) ? '#39ff14' : day ? '#0a0a0a' : 'transparent',
              color: day && workoutDates.includes(day) ? '#0a0a0a' : '#fff',
              borderRadius: '5px',
              fontWeight: day && workoutDates.includes(day) ? 'bold' : 'normal',
  ...
  // Get exercise name from ID for Personal Bests
  ...
  return (
    <div style={{minHeight: '100vh', background: '#0a0a0a', color: '#fff', fontFamily: 'Rajdhani, sans-serif', paddingBottom: '80px'}}>
      ...
      <div style={{padding: '20px', maxWidth: '600px', margin: '0 auto'}}>
                  ...
                  {exercise.sets.map((set, setIndex) => {
                    ...
                    return (
                      <div key={setIndex} style={{display: 'grid', gridTemplateColumns: exercise.trackingType === 'weight_reps' ? '50px 1fr 1fr' : exercise.trackingType === 'reps_only' ? '50px 1fr' : exercise.trackingType === 'duration' ? '50px 1fr 100px' : '50px 1fr', gap: '8px', marginBottom: '8px'}}>
                        ...
                        {/* Duration input with countdown timer and circular progress */}
                        {exercise.trackingType === 'duration' && (
                          <>
                            <input
                              type="number"
          ...
          <div>
            ...
            {sessions.length === 0 ? (
              ...
              sessions.map((session) => {
                // Group exercises to avoid duplicates
        ...
        {currentView === 'reports' && (
          <div>
            ...
            {/* Personal Bests with Integrated 1RM */}
            <div style={{background: '#1a1a1a', padding: '20px', borderRadius: '12px', marginBottom: '20px', border: '1px solid #2a2a2a'}}>
              <h3 style={{color: '#39ff14', marginBottom: '15px', fontFamily: 'Bebas Neue'}}>🏆 PERSONAL BESTS & 1RM</h3>
              {Object.keys(personalBests).length === 0 ? (
                ...
                Object.entries(personalBests).map(([exerciseId, best]) => {
                  ...
                  const oneRM = calculate1RM(best.weight, best.reps);
                      ...
                      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'start'}}>
                        <div>
                          ...
                          <div style={{color: '#39ff14', fontSize: '1rem', marginBottom: '5px'}}>
                            PR: {best.weight}kg × {best.reps} reps
                          </div>
      ...
      {/* Exercise Picker Modal WITH CREATE NEW BUTTON */}
      {showExercisePicker && (
        <div style={{
          position: 'fixed',
          top: 0,
          ...
          <div style={{
            ...
            <div style={{padding: '20px'}}>
              {/* CREATE NEW EXERCISE BUTTON - FIX #1 */}
              <button
                onClick={() => {
                  setShowExercisePicker(false);
