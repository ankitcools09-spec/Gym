...
function GymTracker() {
  ...
  const loadData = async () => {
    try {
      ...
      if (statsRes.ok) {
        const statsData = await statsRes.json(); // FIX: Was sessionsRes.json() - WRONG!
  ...
  // FIXED: Get ALL sets from last session (not just best set)
    ...
    for (let i = 0; i < sessions.length; i++) {
      ...
      if (session.exercises) {
        ...
        if (exerciseData && exerciseData.sets && exerciseData.sets.length > 0) {
          // Return ALL sets, not just the best one
          return {
  ...
  // Workout Calendar with clickable dates
                ...
                <div key={exIndex} style={{
                  ...
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '15px'}}>
                    <div style={{flex: 1}}>
                      ...
                      {/* FIXED: Last Session Stats - ALL SETS */}
          ...
          <div>
            ...
            <div style={{background: '#1a1a1a', padding: '20px', borderRadius: '12px', border: '1px solid #2a2a2a', textAlign: 'center'}}>
              ...
              <div style={{color: '#444', fontSize: '0.8rem', marginTop: '10px'}}>
                ✅ Fixed: PRs Loading ✅ All Sets Shown
              </div>
            </div>
          </div>
        )}
