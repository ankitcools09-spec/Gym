...
function GymTracker() {
  ...
  const [durationTimers, setDurationTimers] = useState({}); // Track timers for duration exercises
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (hasRunningTimer) {
        setDurationTimers(prev => ({ ...prev })); // Force update
      }
    }, 1000);
    return () => clearInterval(interval);
  }, [durationTimers]);
  ...
  const deleteExercise = async (exerciseId) => {
    if (!window.confirm('Delete this exercise? This will remove it from all past workouts.')) return;
    
    try {
      const res = await fetch(`${API_BASE}/exercises?id=${exerciseId}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        await loadData();
      ...
      <div style={{padding: '20px', maxWidth: '600px', margin: '0 auto'}}>
            ...
            {currentSession.exercises.map((exercise, exIndex) => {
              ...
              return (
                <div key={exIndex} style={{
                  ...
                  {/* Dynamic input headers based on tracking type */}
                  <div style={{display: 'grid', gridTemplateColumns: exercise.trackingType === 'weight_reps' ? '50px 1fr 1fr' : exercise.trackingType === 'reps_only' ? '50px 1fr' : exercise.trackingType === 'duration' ? '50px 1fr 80px' : '50px 1fr', gap: '8px', marginBottom: '8px'}}>
                    ...
                    {exercise.trackingType === 'duration' && (
                      <>
                        <div style={{fontWeight: 'bold', color: '#39ff14', fontSize: '0.9rem'}}>DURATION</div>
                  ...
                  {exercise.sets.map((set, setIndex) => {
                    ...
                    return (
                      <div key={setIndex} style={{display: 'grid', gridTemplateColumns: exercise.trackingType === 'weight_reps' ? '50px 1fr 1fr' : exercise.trackingType === 'reps_only' ? '50px 1fr' : exercise.trackingType === 'duration' ? '50px 1fr 80px' : '50px 1fr', gap: '8px', marginBottom: '8px'}}>
        ...
        {currentView === 'reports' && (
          <div>
              ...
              {Object.keys(personalBests).length === 0 ? (
                ...
                Object.entries(personalBests).map(([exerciseName, best]) => (
                  <div key={exerciseName} style={{
                    ...
                    <span style={{color: '#fff'}}>{exerciseName}</span>
            ...
            {/* 1RM Calculator */}
            <div style={{background: '#1a1a1a', padding: '20px', borderRadius: '12px', marginBottom: '20px', border: '1px solid #2a2a2a'}}>
              <h3 style={{color: '#39ff14', marginBottom: '15px', fontFamily: 'Bebas Neue'}}>🔢 1RM CALCULATOR</h3>
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '15px'}}>
                <input
                  ...
                  style={{background: '#0a0a0a', border: '2px solid #2a2a2a', color: '#fff', padding: '12px', borderRadius: '8px'}}
                />
                <input
                  type="number"
                  placeholder="Reps"
              ...
              {oneRMInput.weight && oneRMInput.reps && (
                <div style={{textAlign: 'center', padding: '20px', background: '#0a0a0a', borderRadius: '8px'}}>
                  <div style={{color: '#666', fontSize: '0.9rem', marginBottom: '5px'}}>Estimated 1RM:</div>
                  <div style={{fontSize: '2.5rem', color: '#39ff14', fontFamily: 'Bebas Neue'}}>
                    {Math.round(oneRMInput.weight * (1 + oneRMInput.reps / 30))} kg
                  </div>
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div style={{background: '#1a1a1a', padding: '20px', borderRadius: '12px', border: '1px solid #2a2a2a', textAlign: 'center'}}>
              ...
              <div style={{color: '#666', fontSize: '0.9rem'}}>
                Database Version • Enhanced Features
              </div>
