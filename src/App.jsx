...
function GymTracker() {
  ...
  // FIXED: Calendar with proper date normalization
  const WorkoutCalendar = () => {
    ...
    const handleDateClick = (day) => {
      ...
      const hasWorkout = workoutDates.some(wd => wd.day === day);
      if (hasWorkout) {
    ...
    return (
      <div style={{ background: '#1a1a1a', padding: '20px', borderRadius: '12px', marginBottom: '20px', border: '1px solid #2a2a2a' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '5px', textAlign: 'center' }}>
          ...
          {days.map((day, idx) => {
            const hasWorkout = day && workoutDates.some(wd => wd.day === day);
  ...
  return (
    <div style={{minHeight: '100vh', background: '#0a0a0a', color: '#fff', fontFamily: 'Rajdhani, sans-serif', paddingBottom: '80px'}}>
      ...
      {/* NEW: Floating Add Exercise Button - Bottom Left */}
      {currentView === 'log' && (
        <button
          ...
          style={{
            ...
            fontSize: '2rem',
            cursor: 'pointer',
            boxShadow: '0 5px 20px rgba(57, 255, 20, 0.5)',
            zIndex: 1000,
            display: 'flex',
            alignItems: 'center',
                      ...
                      {lastSession && lastSession.sets && lastSession.sets.length > 0 && (
                        <div style={{
                          ...
                          <div style={{color: '#888', fontSize: '0.75rem', marginBottom: '5px'}}>
                            Last session: {new Date(lastSession.date).toLocaleDateString('en-US', {month: 'short', day: 'numeric'})}
                          </div>
                          <div style={{color: '#00bfff', fontSize: '0.85rem'}}>
                            {lastSession.sets.map((set, idx) => {
                              let setText = '';
                              if (exercise.trackingType === 'weight_reps') {
                                setText = `${set.weight}kg × ${set.reps}`;
                              } else if (exercise.trackingType === 'reps_only') {
                                setText = `${set.reps} reps`;
                              } else if (exercise.trackingType === 'duration') {
                                setText = formatDuration(set.duration);
                              } else if (exercise.trackingType === 'distance') {
                                setText = formatDistance(set.distance);
                              }
                              return (
                                <div key={idx} style={{marginBottom: '3px', fontWeight: idx === 0 ? 'bold' : 'normal'}}>
                                  Set {idx + 1}: {setText}
                                </div>
                              );
                            })}
            ...
            {sessions.length === 0 ? (
              ...
              sessions
                .filter(session => {
                  ...
                  const sessionNormalized = normalizeDate(session.date);
                  return sessionNormalized === selectedDate;
                })
                .map((session) => {
                  const uniqueExercises = {};
                  if (session.exercises) {
          ...
          <div>
            ...
            <div style={{background: '#1a1a1a', padding: '20px', borderRadius: '12px', border: '1px solid #2a2a2a', textAlign: 'center'}}>
              ...
              <div style={{color: '#444', fontSize: '0.8rem', marginTop: '10px'}}>
                ✅ Floating Button ✅ Calendar Fixed
              </div>
            </div>
          </div>
        )}
