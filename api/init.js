...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Create indexes
    await sql`CREATE INDEX IF NOT EXISTS idx_sessions_date ON sessions(date DESC)`;
    await sql`CREATE INDEX IF NOT EXISTS idx_session_sets_session ON session_sets(session_id)`;
    await sql`CREATE INDEX IF NOT EXISTS idx_session_sets_exercise ON session_sets(exercise_id)`;
    await sql`CREATE INDEX IF NOT EXISTS idx_exercises_tracking_type ON exercises(tracking_type)`;

    // Check if exercises already exist
    const existingExercises = await sql`SELECT COUNT(*) as count FROM exercises`;
    const currentCount = parseInt(existingExercises.rows[0].count);
    
    if (currentCount === 0) {
      // Insert all 70 exercises
      const exercises = [
        // CHEST
      ...
      for (const [name, muscleGroup, equipmentType, trackingType] of exercises) {
        await sql`
          ...
          VALUES (${name}, ${muscleGroup}, ${equipmentType}, ${trackingType})
        `;
      }
    }
    ...
    const totalExercises = parseInt(finalCount.rows[0].count);
